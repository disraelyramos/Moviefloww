...
export default function GestionLotes({
  ...
  const validar = () => {
    ...
    if (!soloInfo) {
      ...
      // Cantidad:
      //   - CREAR: obligatoria > 0
      //   - EDITAR: OPCIONAL; si viene, debe ser > 0
      if (esCrear) {
      ...
      if (values?.fechaVencimiento) {
        ...
        fv.setHours(0,0,0,0);
        if (Number.isNaN(fv.getTime())) e.fechaVencimiento = "Fecha inválida (use YYYY-MM-DD)";
        else if (fv < hoy) e.fechaVencimiento = "No puede ser anterior a hoy";
      }
    }
  ...
  const loteSeleccionado = lotes.find((l) => String(l.ID) === String(values?.numeroLoteId));
  const textoLote =
    loteSeleccionado
      ? `${loteSeleccionado.ID} — ${loteSeleccionado.CODIGO_LOTE ? `[${loteSeleccionado.CODIGO_LOTE}] ` : ""}${loteSeleccionado.NOMBRE || ""}`
      : values?.numeroLoteId
          ...
          <div className="col-12 col-lg-6">
            <div className="border rounded-3 p-3 h-100">
              ...
              <div className="mb-3">
                ...
                {soloInfo ? (
                  <input type="text" className="form-control" value={textoLote || "N/A"} readOnly />
                ) : (
                  <select
                    name="numeroLoteId"
                  ...
                  >
                    ...
                    {lotes.map((l) => (
                      <option key={l.ID} value={l.ID}>
                        {l.ID} — {l.CODIGO_LOTE ? `[${l.CODIGO_LOTE}] ` : ""}{l.NOMBRE}
                      </option>
                    ))}
                  </select>
                )}
                {errors.numeroLoteId && (
                  <div className="invalid-feedback d-block">{errors.numeroLoteId}</div>
                )}
              </div>
              ...
              <div className="mb-0">
                ...
                {soloInfo ? (
                  <input
                    ...
                    value={values?.fechaVencimientoDisplay || values?.fechaVencimiento || "N/A"}
                    readOnly
                  />
                ) : (
                  <input
                ...
                {errors.fechaVencimiento && (
                  <div className="invalid-feedback d-block">{errors.fechaVencimiento}</div>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="mb-3">
                ...
                {soloInfo ? (
                  ...
                  <input
                    ...
                    onChange={handle}
                    placeholder={esCrear ? "0 (obligatorio, > 0)" : "opcional (se sumará al stock)"}
                    min={esCrear ? 1 : undefined}
                  />
                )}
                {errors.cantidad && (
                  <div className="invalid-feedback d-block">{errors.cantidad}</div>
                )}
                {!soloInfo && (
