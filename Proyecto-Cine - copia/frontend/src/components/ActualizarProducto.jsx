...
const EditarProductoModal = ({
  ...
  useEffect(() => {
    ...
    // Precargar lote del producto
    (async () => {
      setLotesLoading(true);
      setLotesError("");
      try {
        const res = await axios.get(`${API_BASE}/producto-por-lote`, {
        ...
        if (items.length === 0) {
          ...
          setLoteForm({
            idPorLote: first.id,
            numeroLoteId: first.loteId,
            fechaVencimiento: toISO(first.fechaVencimiento),
            cantidad: "",
          });
        }
      } catch (err) {
  ...
  const handleGuardarTodo = async () => {
    ...
    try {
      ...
      // 1) PUT producto (solo los campos enviados) — sin precioCosto
      ...
      if (Object.keys(payloadPPL).length > 0) {
        await axios.put(`${API_BASE}/producto-por-lote/${loteForm.idPorLote}`, payloadPPL);
      }
