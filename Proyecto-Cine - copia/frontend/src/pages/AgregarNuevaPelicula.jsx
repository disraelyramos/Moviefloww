// src/pages/Peliculas.jsx  (el archivo que me compartiste)
// Lista las tarjetas y abre los modales de agregar/editar.
// Ajustado: se quitaron precio, fecha/hora y sala del render.
...
const Peliculas = () => {
  ...
  // Al guardar desde el modal de "Agregar"
  ...
  // NUEVO: abrir confirm con pre-validación
  const abrirEliminar = async (p) => {
    setChecking(true);
    try {
      // Si existe este endpoint, úsalo para contar funciones de la película
      ...
      if (count > 0) {
        toast.info(`No se puede eliminar "${p.titulo}": tiene ${count} función(es) programadas.`);
        return;
      }
    } catch {
      // Si no existe el endpoint, continuamos: el backend lo validará al eliminar
    } finally {
      setChecking(false);
    }
...
// NUEVO: ejecutar eliminación
  ...
  // Actualizado desde modal de edición
  const onActualizado = (upd) => {
    setPeliculas(prev => prev.map(x => (String(x.id) === String(upd.id) ? upd : x)));
    setEditOpen(false);
    setEditData(null);
  ...
  return (
    <div className="container py-4">
      ...
      <div className="d-flex align-items-start justify-content-between gap-3 mb-4 flex-wrap">
        ...
        {/* Derecha: botón arriba, filtro abajo */}
              ...
              <div className="card h-100 shadow-sm border-0 position-relative">
                ...
                <div className="card-body pb-5">
                  ...
                  <div className="mb-2 d-flex flex-wrap gap-2">
                    {p.formatoNombre && (
                      <span className="badge bg-secondary">{p.formatoNombre}</span>
                    )}
                    {p.idiomaNombre && (
                      <span className="badge bg-info text-dark">{p.idiomaNombre}</span>
                    )}
                    {p.clasificacionCodigo && (
                      <span className="badge bg-dark">{p.clasificacionCodigo}</span>
                    )}
                  </div>

                  {p.duracionMin != null && (
                    <div className="text-muted mb-1">{p.duracionMin} min</div>
                  )}
                  {p.categoriaNombre && (
                    <div className="text-muted mb-1">{p.categoriaNombre}</div>
                  )}
                </div>
              </div>
            </div>
                    ...
                    <div className="d-flex align-items-start gap-3">
                      ...
                      <div className="flex-grow-1">
                        ...
                        <p className="text-muted small mb-1">
                          Vas a eliminar: <strong>{peliculaAEliminar?.titulo}</strong>.
                        </p>
